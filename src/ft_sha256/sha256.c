...
static void init_w_array(t_64_uint32 w_array, unsigned char *msg_buffer)
{
    int i;
...
static void sha256_tranform_buffers(t_8_uint32 internal_buffers,
    t_64_uint32 w_array)
{
    int			i;
    uint32_t	temp_a;
    ...
    while (i < 64) {
        temp_a = internal_buffers[7] + sha256_op_b(
            ...
            internal_buffers[5],
            internal_buffers[6]) + g_sha256_k[i] + w_array[i];
