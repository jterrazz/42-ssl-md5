...
/*   Updated: 2019/05/19 23:34:36 by jterrazz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void sha256_tranform_buffers(t_8i_buffer internal_buffers, t_64i_buffer w_array) {
    int i;
    unsigned int temp_a;
    unsigned int temp_b;
    ...
    i = 0;
    while (i < 64) {
        temp_a = internal_buffers[7] + sha256_op_b(internal_buffers[4]) + sha256_op_ch(internal_buffers[4], internal_buffers[5], internal_buffers[6]) + g_sha256_k[i] + w_array.i[i];
...
static void sha256_run_ops(t_8i_buffer buffers, unsigned char *msg_buffer, size_t msg_len) {
    size_t chunk_i;
