...
/*   Updated: 2019/07/27 12:39:49 by jterrazz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sha256_shuffle_buffers(t_8_uint32 buffers, t_64_uint32 w_array)
{
	...
	while (i < 64)
	{
		temp_a = buffers[7] + sha256_op_b(
			buffers[4]) + sha256_op_ch(buffers[4], buffers[5],
			buffers[6]) + g_sha256_k[i] + w_array[i];
		temp_b = sha256_op_a(buffers[0]) + sha256_op_maj(buffers[0],
			buffers[1], buffers[2]);
		buffers[7] = buffers[6];
		buffers[6] = buffers[5];
		buffers[5] = buffers[4];
	...
	unsigned char *formatted_msg, size_t msg_len)
{
	...
	{
		sha256_init_w_array(w_array, formatted_msg +
			chunk_i * SHA256_C_SIZE);
