...
/*   Updated: 2019/07/27 12:28:56 by jterrazz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "./md5.h"

static const t_ops_buffer g_ft_f[4] =
{ &md5_op_shift_1, &md5_op_shift_2, &md5_op_shift_3, &md5_op_shift_4 };

static const t_ops_1a g_ft_g[4] =
...
static void	md5_shuffle_buffers(int i, t_16_uint32 chunk, t_4_uint32 buffers)
{
	...
	g = g_ft_g[i / 16];
	f = g_ft_f[i / 16];
	f_result = f(buffers) + buffers[0] + g_computed_sines[i] + chunk[g(i)];
	buffers[0] = buffers[3];
	buffers[3] = buffers[2];
	buffers[2] = buffers[1];
	buffers[1] = buffers[1] + ft_brotate_left(f_result, g_md5_shifts[i]);
}
